...
<div class="panel panel-primary">
	...
	<div class="panel-heading panel-primary">
		...
		<h5><warehouse:message code="Recent Messages" args="[session.warehouse.name]"/></h5>
	</div>
	<div class="panel-body">		<%-- 	
		<div style="padding: 10px">
			There are ${messagesList.size() } recent activities.		
		</div>
		--%>

		<table class="table table-bordered">
		<tbody>
                    <g:if test="${messagesList}">
                        <tr>
                            <td colspan="2">
                                <div >
                                    <warehouse:message code="Showing Recent Messages" args="[startIndex+1,endIndex+1,messagesListTotal,daysToInclude]"/>
                                </div>
                            </td>
                        </tr>
                    </g:if>
					<g:set var="status" value="${0 }"/>
		 			<g:each var="activity" in="${messagesList }" status="i">
		 				<tr class="${status++%2?'even':'odd' } prop">
							...
							<td class="top">
		 						<div>${activity.label }</div><span class="small text-muted">${format.date(obj:activity.lastUpdated,format:'MMMMM dd hh:mm a')}</span>
		 					</td>
		 				</tr>
		 			</g:each>
		 			
		 			<g:unless test="${messagesList}">
						<tr class="">
							<td class="center">
		...
		<g:if test="${messagesList}">
			<div class="paginateButtons">
				<g:paginate total="${messagesListTotal}" />
			</div>
		</g:if>
	</div>
</div>	
