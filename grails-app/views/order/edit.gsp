...
<%--<div style="float:left;"><g:render template="chat" model="[orderInstance:order]"/></div>
	...
	--%><div >
		...
		<div class="content_inner"><label class="pagetitle"><warehouse:message code="order.editpo.label"/></label></div>
			
			<input type="hidden" name='id' value="${orderInstance?.id?.encodeAsHTML()}"/>
				<fieldset>
				<div class="right">
            		...
            		<table>
						...
						<td><input type="text" name='orderNumber' value="${orderInstance?.orderNumber?.encodeAsHTML()}" size="25" readonly="readonly"/></td>
						<td style=""><label for='bu'><warehouse:message code="order.bu.label" default="BU"/></label></td>
						<td><g:selectBU name="bu" optionKey="bu" value="${order?.bu}" />
						<td style=""><label for='status'><warehouse:message code="order.status.label"/></label></td>
						<td><input type="text" name='status' value="${orderInstance?.status}"  size="25" readonly="readonly"/></td>
						...
						<tr>
								...
								<td>
								<input type="text" name="dateOrdered" id="dateOrdered"
								
									value="${formatDate(format:'MM/dd/yyyy', date:orderInstance?.dateOrdered )}"> <script
								...
								<%--<g:jqueryDatePicker id="dateOrdered" name="dateOrdered"
								...
								<td>
								<input type="text" name="dueDate" id="dueDate" value="${formatDate(format:'MM/dd/yyyy', date:orderInstance?.dueDate )}">
									<script type="text/javascript">
										$(function(){
										$('*[name=dateOrdered]').datepicker({
											 "closeOnSelected": true,
											 "format":'m/d/Y'
											 });
										});
					...
					<table>
								...
								<table style="border: 1px solid lightgrey">
									<tr>
										...
										--%> <span id="origin-span" class="span"
											...
											id="origin-value" class="value" type="hidden" value="${orderInstance?.origin?.id}"
											name="origin"> <input id="fromAddress.id"
											class="value" type="hidden" value="${orderInstance?.fromAddress?.id}" name="fromAddress.id">
									...
									<tr>
										...
										<td><input type="text" id='fromAddress.postalCode' name='fromAddress.postalCode'
											value="${orderInstance?.fromAddress?.postalCode?.encodeAsHTML()}" />
									</tr>
									<tr>
										<td style=""><label>Pickup Start</label></td>
										<td>
										<input type="text" name="pickupStart" id="pickupStart" value="${formatDate(format:'MM/dd/yyyy HH:mm', date:orderInstance?.pickupStart) }">
									<script type="text/javascript">
										$(function(){
									...
									<tr>
										...
										<td>
										<input type="text" name="pickupEnd" id="pickupEnd" value="${formatDate(format:'MM/dd/yyyy HH:mm', date:orderInstance?.pickupEnd )}">
									<script type="text/javascript">
										$(function(){
								...
								<table style="border: 1px solid lightgrey">
									<tr>
										...
										<td>
											...
											style="text-align: left; display: none;"></span> <input
											id="destination-value" class="value" type="hidden" value="${orderInstance?.destination?.id}"
											name="destination"> <input id="toAddress.id"
											class="value" type="hidden" value="${orderInstance?.toAddress?.id}" name="toAddress.id">
											<span class="ui-helper-hidden-accessible" role="status"
									...
									<tr>
										<td style=""><label for='postalCode'>Zip Code</label></td>
										<td><input type="text" id='toAddress.postalCode' name='toAddress.postalCode'
											value="${orderInstance?.toAddress?.postalCode?.encodeAsHTML()}" />
									</tr>
									<tr>
									<td style=""><label>Delivery Start</label></td>
									<td>
									<input type="text" name="deliveryStart" id="deliveryStart" value="${formatDate(format:'MM/dd/yyyy HH:mm', date:orderInstance?.deliveryStart )}">
									<script type="text/javascript">
										$(function(){
									...
									<%--<g:jqueryDatePicker 
										...
										<td>
										<input type="text" name="deliveryEnd" id="deliveryEnd" value="${formatDate(format:'MM/dd/yyyy HH:mm', date:orderInstance?.deliveryEnd )}">
									<script type="text/javascript">
										$(function(){
						...
						<tr class="prop">
						   ...
						   <td valign="top" class="dtl-header">
							...
							<td valign="top" class="dtl-header-small">
							<label class="section-header" for="RTS"><warehouse:message code="rts.label" default="RTS" /></label>
							</td>
							<td valign="top" class="dtl-header-small"></td>
							</tr> 
							</table>
							<table>
						  		<tr valign="top" class="value ${hasErrors(bean: orderInstance, field: 'orderItems', 'errors')}">
							       ...
							       <g:render template="orderItemsEdit" model="['orderInstance':orderInstance]" />
							       <!-- Render the orderItems template (_orderItems.gsp) here -->
							   	</tr>
						...
						<div style="font-weight:bold;font-size:16px;color:gray;"><label for='rtslines'><warehouse:message code="default.rtslines.label" default="RTS Lines"/></label></div>
						<table>										
						<tr class="prop">
						   <td valign="top" class="dtl-header">
						   ...
						   <td valign="top" class="dtl-header">
							<label for="productName"><warehouse:message code="orderItemRTS.productClass.label" default="Product Class" /></label>
							</td>
							<td valign="top" class="dtl-header">
						...
						<tr class="prop">
		                        	...
		                        	<td valign="top">
						                <div class="action-menu" >
											<g:render template="actionsEdit" model="[orderInstance:orderInstance]"/>
										</div>                 	
		                        	</td>
		                        </tr>
						</table>				
	...
	<div id="dialog-form" title="Mark item ready to ship">
        <form id="rtsForm">
          ...
          <div class="bootstrap">
          	...
          	<input type="hidden" name="itemid" id="itemid" value="${orderItem?.lineNum}"/>
 ...
 <script>
	   <g:remoteFunction controller="message" action="setReceipient" params="{companyId:orderInstance.seller.id}"/>
...
<div id="dialog-collab" title="Cim Collab">
    ...
    <g:render template='orderItemEdit' model="['orderItem':null,'i':'_clone','hidden':true]"/>
    <!-- Render the orderItem template (_orderItem.gsp) hidden so we can clone it -->
</body>
</div>
</html>