<script type="text/javascript">
    ...
    function addAsnItem(){
    	var lineNum = childCount + 1;
    //   alert( ${asnInstance?.asnItems?.size()});
    ...
    $('.del-asnItemEdit').live('click', function() {
        //find the parent div
        var prnt = $(this).parents(".asnItemEdit-div");
...
<div id="childList">
    <g:each var="asnItem" in="${asnInstance.asnItems}" status="i">
        ...
        <g:render template='asnItemEdit' model="['asnItem':asnItem,'i':i,'hidden':false]"/>
        <!-- Render the asnItem template (_asnItem.gsp) here -->
    </g:each>
