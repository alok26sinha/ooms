					...
					<td>
						...
						<div>
							...
							<span class="ordered-by"><warehouse:message code="order.orderedBy.label"/>: 
								...
								<g:if test="${orderInstance?.destination }">(${orderInstance?.destination?.name })</g:if>
							</span>
						</div>
					</td>										
					<td style="text-align: right;">
						...
						<g:if test="${!params.execution && !isAddingComment && !isAddingDocument}">						
							...
							<g:elseif test="${!orderInstance?.isReceived() && orderInstance?.isPlaced() }">
			                                    <g:isUserNotInRole roles="[RoleType.ROLE_SUPPLIER]">
								...
								<g:link controller="order" action="acceptOrder" id="${orderInstance?.id}">
									<button>
										${warehouse.message(code: 'order.acceptOrder.label')}
									</button> 
								</g:link>
							</g:elseif>
