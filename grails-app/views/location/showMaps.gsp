  ...
  <head>
    ...
    'view': {'columns': [0, 3]}
      ...
      //  google.visualization.events.addListener(weekComboChart, 'select', selectHandler);
        ...
        var weekChrt = new google.visualization.Dashboard(document.getElementById('weekDashboard')).bind(weekRangeSlider, weekComboChart).draw(weekData);
        var weekSlider = new google.visualization.Dashboard(document.getElementById('weekDashboard')).bind(weekskuPicker, weekComboChart).draw(weekData);


        

       	var charty =  new google.visualization.Dashboard(document.getElementById('dayDashboard')).bind(dayRangeSlider, dayComboChart).draw(dayData);
       	
        var chrt = new google.visualization.Dashboard(document.getElementById('dayDashboard')).bind(dayskuPicker, dayComboChart)
        ...
        google.visualization.events.addListener(dayComboChart, 'select', function() {
      	 ...
      	 google.visualization.events.addListener(weekComboChart, 'select', function() {
			
      		chartObject = weekComboChart.getChart();
      		dispDivChartDetail(chartObject);
      	});
...
<script>
	...
	--%><div><table><tr><td style="width:35%"><table>
							...
							<td><g:selectProductGroup name="productGroup" optionKey="id" value="${productGroup}" noSelection="['':warehouse.message(code:'default.all.label')]" /></td>
          					
							</tr>
							<tr>
							<td align="right"><label class="select">Select SKU</label></td>
							<td><g:selectProduct name="product" optionKey="id" value="${product}" noSelection="['':warehouse.message(code:'default.all.label')]" />	
							</td>
							</tr>
							</table>
							</td><td align="left">
							...
							<label class="select">Select SKU</label><g:select name="skuLst" id="skuLst" from="${skuList}"	 onchange="${remoteFunction(controller: 'Location', action: 'showMap', params: "'skuList=' + this.value + '&&id=' + '${params.id}'" ,oncomplete='showSpinner();')}" /></div>
							 --><div id="map_div" style="width: 800px; height: 600px;top:270px;"></div>
