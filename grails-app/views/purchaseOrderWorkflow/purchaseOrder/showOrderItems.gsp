								...
								<table style="">
									<thead>
										<tr class="odd">
											...
											<g:each var="uomObject" in="${orderSizesInherited}" status="orderSizesInheritedStatus">	 	  	
												<g:sortableColumn class="right" property="${uomObject?.description}" title="${uomObject?.description}"/>			
											</g:each>
									...
									<tbody>
										...
										<g:each var="orderItem" in="${order?.orderItems}">
											<tr class="${(i++ % 2) == 0 ? 'even' : 'odd'}">
												...
												<g:each var="inheritOnOrderUOM" in="${orderSizesInherited}" status="orderSizesInheritedStatus">
										...
										<tfoot>
										<g:set var="x" value="${orderSizesInherited.size + 5}"/>
											<tr class="${(i++ % 2) == 0 ? 'even' : 'odd'}">
												<td colspan="${x}"></td>
										...
										<g:form action="purchaseOrder" method="post">
											...
											<table>
												<tbody>
													...
													<g:each var="inheritOnOrderUOM" in="${orderSizesInherited}" status="orderSizesInheritedStatus">
													<tr class='prop'>
														<td valign='top' class='name'><label for="${inheritOnOrderUOM?.id }">${inheritOnOrderUOM?.description}</label></td>
														<td valign='top' class='value'>
															<g:set var="sizevariable" value="${inheritOnOrderUOM?.inheritOnOrder}"/>
															<g:if test="${sizevariable==com.cimbidia.wm.core.OrderSizes.SIZE1}">
																<input type="text" id="size1" name="size1" value="${orderItem?.size1}" size="10" class="text" />
															</g:if>
															<g:elseif test="${sizevariable==com.cimbidia.wm.core.OrderSizes.SIZE2}">
																<input type="text" id="size2" name="size2" value="${orderItem?.size2}" size="10" class="text" />
															</g:elseif>
															<g:else test="${sizevariable==com.cimbidia.wm.core.OrderSizes.SIZE3}">
																<input type="text" id="size3" name="size3" value="${orderItem?.size3}" size="10" class="text" />
															</g:else>
														</td>
													</tr>
													</g:each>
