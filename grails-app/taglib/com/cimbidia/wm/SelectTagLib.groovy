...
import com.cimbidia.wm.core.ReasonCode
...
class SelectTagLib {
	...
	def selectCarrierZone = { attrs, body ->
		attrs.from = Company.list().sort { it?.name?.toLowerCase() }
		attrs.optionKey = 'id'
		attrs.value = attrs.value
		attrs.optionValue = { it.name }
		out << g.select(attrs)
	...
	def selectOceanCarrier = { attrs, body ->
		def mot = Mot.findAllByCode(Mot.OCN);
		println "mot found is ="+mot
		if(mot)
		{
			def carrierMotList = CarrierMot.findAllByMot(mot);
			def companyInstanceList = new ArrayList();
			println "carrierMotList.size=" + carrierMotList.size()
			for (int i=0;i < carrierMotList.size();i++)
			{
				def companyInstance = carrierMotList[i].company
				companyInstanceList.add(companyInstance)
				
			}
			attrs.from = companyInstanceList.sort { it?.companyName?.toLowerCase() }
		}
		else
		{
		println "mot not found"
		}
		
		attrs.optionKey = 'id'
		attrs.value = attrs.value
		attrs.optionValue = { it.companyName }
		out << g.select(attrs)
	}
	
	void createConstantsList(attrs, type, sortOrder, sortBy)
	{

		attrs.from = Constants.createCriteria().list{ eq("type", type) order(sortBy?:"sortOrder" , sortOrder?:"asc")}
	...
	void createConstantsList(attrs, type)
	{
		createConstantsList(attrs, type, null, null)
	}
	
	void createConstantsList(attrs, type, sortOrder)
	{
		createConstantsList(attrs, type, sortOrder, null)
	}
		
	def selectIncoterm = { attrs, body ->
		createConstantsList(attrs,"INCO");
		out << g.select(attrs)
	}
	
	def selectUNNumber = { attrs, body ->
		createConstantsList(attrs,"UNNUM");
		out << g.select(attrs)
	}
	
	def selectSHC = { attrs, body ->
		createConstantsList(attrs,"SHC");
		out << g.select(attrs)
	}
	
	def selectTranspService = { attrs, body ->
		createConstantsList(attrs,"TRANSP_SERV");
		out << g.select(attrs)
	}
	
	def selectEquipProvider = { attrs, body ->
		createConstantsList(attrs,"EQPROV");
		out << g.select(attrs)
	}
	
	def selectUOM = { attrs, body ->
		createConstantsList(attrs,"UOM");
		out << g.select(attrs)
	}
	
	def companyType = {attrs, body ->
		createConstantsList(attrs,"COMP");
		out << g.select(attrs)
	}
	
	def selectPUOM = { attrs, body ->
		createConstantsList(attrs,"PUOM");
		out << g.select(attrs)
	}
	def selectPC = { attrs, body ->
		createConstantsList(attrs,"PC");
		out << g.select(attrs)
	}
	
	//Temperature control
	def selectTempCtrl = { attrs, body ->
		createConstantsList(attrs,"TEMP_CTRL");
		out << g.select(attrs)
	}
	
	//Transportation Resposibility
	def selectTR = { attrs, body ->
		createConstantsList(attrs, "TR")
		out << g.select(attrs)
	}
	def selectCOMP = { attrs, body ->
		createConstantsList(attrs, "COMP")
		out << g.select(attrs)
	}
	
	def selectCommodity = { attrs, body ->
		createConstantsList(attrs, "COMMODITY")
		out << g.select(attrs)
	}
	
	def selectRateType = { attrs, body ->
		createConstantsList(attrs,"RATE_TYPE");
		out << g.select(attrs)
	}
	
	def selectRateUnit = { attrs, body ->
		createConstantsList(attrs,"RATE_UNIT");
		out << g.select(attrs)
	}
	
	def selectModifReason = { attrs, body ->
		createConstantsList(attrs,"RATE_MOD_REASON");
		out << g.select(attrs)
	}
	
	def selectSTATE = { attrs, body ->
		attrs.from = Constants.createCriteria().list{ eq("type", 'STATE') order("sortOrder", "asc")}
	...
	//Frequency
	def selectFreq = { attrs, body ->
		createConstantsList(attrs, "FREQ")
		out << g.select(attrs)
	}
		
	//Service Level
	def selectSL = { attrs, body ->
		createConstantsList(attrs, "SL")
		out << g.select(attrs)
	}
		
	def selectEQUIP = { attrs, body ->
		attrs.from = Equipment.list()
		attrs.optionKey = 'id'
		attrs.value = attrs.value
	...
	def selectLtlZone = { attrs, body ->
		attrs.from = LtlRatesZone.list()
		attrs.optionKey = 'name'
		attrs.value = attrs.value
		attrs.optionValue = { it.name }
		out << g.select(attrs)
	}
	def selectCMDT = { attrs, body ->
	...
	def selectProduct = { attrs, body ->
		attrs.from = Product.createCriteria().list{ /*or{eq("id", session?.companyId?session?.companyId:1) eq("parentCompanyId", session?.companyId?session?.companyId:1) }*/}
		attrs.optionKey = 'id'
		attrs.value = attrs.value
		attrs.optionValue = { it.name }
		out << g.select(attrs)
	...
	def selectOrder = { attrs, body ->
		attrs.from = Order.createCriteria().list{ /*or{eq("id", session?.companyId?session?.companyId:1) eq("parentCompanyId", session?.companyId?session?.companyId:1) }*/}
		attrs.optionKey = 'id'
		attrs.value = attrs.value
		attrs.optionValue = { it.orderNumber }
		out << g.select(attrs)
	}
	def selectProductGroup = { attrs, body ->
		attrs.from = ProductGroup.createCriteria().list{/* eq("company.id", session?.companyId?session?.companyId:1) */}
		attrs.optionKey = 'id'
		attrs.value = attrs.value
		attrs.optionValue = { it.description }
		out << g.select(attrs)
	}
	
	def selectRatingLaneCarrier = { attrs, body ->
		attrs.from = RatingLaneCarrier.createCriteria().list{/* eq("company.id", session?.companyId?session?.companyId:1) */}
		attrs.optionKey = 'id'
		attrs.value = attrs.value
		attrs.optionValue = { it.id }
		out << g.select(attrs)
	}
	def selectBU = { attrs, body ->
		attrs.from = Company.createCriteria().list{ or{eq("id", session?.companyId?session?.companyId:1) eq("parentCompanyId", session?.companyId?session?.companyId:1) }}
		attrs.optionKey = 'id'
		attrs.value = attrs.value
		attrs.optionValue = { it.companyName }
		out << g.select(attrs)
	}
	def selectCompany = { attrs, body ->
		attrs.from = Company.list().sort { it?.companyName?.toLowerCase() }
		attrs.optionKey = 'id'
		attrs.value = attrs.value
		attrs.optionValue = { it.companyName }
		out << g.select(attrs)
	}
	def selectVendor = { attrs, body ->
		attrs.from = Company.createCriteria().list{ eq("type.id", 9)  }
		attrs.optionKey = 'id'
		attrs.value = attrs.value
		attrs.optionValue = { it.companyName }
		out << g.select(attrs)
	}
	def selectCarrier = { attrs, body ->
		attrs.from = Company.createCriteria().list{ eq("type.id", 10)  }
		attrs.optionKey = { it.id }
		attrs.value = attrs.value
		attrs.optionValue = { it.companyName }
		out << g.select(attrs)
	}
	...
	def selectPort = { attrs, body ->
		attrs.from = Location.list().sort()
		attrs.optionKey = 'id'
		attrs.value = attrs.value
		attrs.optionValue = { it.name }
		out << g.select(attrs)
	...
	def selectLocation = { attrs, body ->
		attrs.from = Location.createCriteria().list{ eq("company.id", session?.companyId?session?.companyId:1) }
		attrs.optionKey = 'id'
		attrs.value = attrs.value
		attrs.optionValue = { it.name }
		out << g.select(attrs)
	}
	def selectLocationType = { attrs, body ->
		attrs.from = LocationType.createCriteria().list{/* eq("company.id", session?.companyId?session?.companyId:1) */}
		attrs.optionKey = 'id'
		attrs.value = attrs.value
		attrs.optionValue = { it.description }
		out << g.select(attrs)
	...
	def selectLocation1 = { attrs,body ->
	...
	def selectCustomer = { attrs,body ->
		attrs.from = Company.createCriteria().list{ eq("type.id", 11)  }
		attrs.optionKey = 'id'
		attrs.value = attrs.value
		attrs.optionValue = { it.companyName }
		out << g.select(attrs)
