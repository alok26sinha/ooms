...
class RoleFilters {
    ...
    def static changeActions = ['edit', 'create', 'process', 'save',
    ...
    def static adminActions = ['console':['index','execute'], 'product': ['create'], 'person': ['list'], 'user': ['list'], 'location': ['edit'], 'shipper': ['create'], 'locationGroup': ['create'], 'locationType': ['create'], '*': ['deleteTODO']]
    def filters = {
        readonlyCheck(controller: '*', action: '*') {
            before = {
                ...
                log.info "Inside ROLEFILTERS === " + missBrowser
