...
class OrderService {
	...
	List<Order> getOrdersPlacedByLocation(Integer companyId,Integer seller, Location origin, Location destination, OrderStatus status, Date orderedFromDate, Date orderedToDate,String orderBy, String colName) {
		
		def companyInstance = Company.findById(companyId)
		def orders = Order.withCriteria {
			
			and {
				if(companyId && companyInstance.type.id==8){ eq("company.id", companyId)}
	...
	List<Order> getOrders(Integer companyId, String orderNumber, Integer seller, java.util.List<Location> locations,OrderStatus status,Date orderedFromDate, Date orderedToDate, String orderBy, String colName) {
	...
	Order saveOrder(Order order) { 		
		...
		if (!order.orderNumber) {
			...
			order.sequence = identifierService.generateOrderIdentifier(order.company.id)
			order.orderNumber = Company.get(order.company.id).companyName + order.sequence
		}
