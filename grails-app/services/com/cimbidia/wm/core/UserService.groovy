...
class UserService {
    ...
    Boolean isUserSupplier(User u) {
        if (u) {
            def user = User.get(u.id)
            def roles = [RoleType.ROLE_SUPPLIER]
            return effectRoles(user).any { 
            		roles.contains(it.roleType) 
            	}
        }
        return false;
    }

	Boolean isUserNotCarrier(User u) {
	        if (u) 
	        {
	        	return (!isUserCarrier(u));
	        }
	        
	       /*     def user = User.get(u.id)
	            def roles = [RoleType.ROLE_CARRIER]
	            def containsCarrierRole = effectRoles(user).any { 
	            		roles.contains(it.roleType) 	
	            	}
	            if(containsCarrierRole)
	            	return false;
	        }
	        return true; */
    }
	...
	Boolean isUserCarrier(User u) {
        ...
        /*  def roles = [RoleType.ROLE_CARRIER]
            ...
            if(containsCarrierRole && user.shipper) */
            
            def companyId = user?.companyId
            if(companyId)
            {
            	Company company = Company.findById(companyId)
            	if(company?.type?.code.equals("3"))
            	{	
            		println "user belongs to carrier Company"
         			return true;
         		}
         	}
        }
    ...
    Boolean canUserBrowse(User u) {
        if (u) {
            def user = User.get(u.id)
            def roles = [RoleType.ROLE_ADMIN, RoleType.ROLE_MANAGER, RoleType.ROLE_BROWSER, RoleType.ROLE_ASSISTANT, RoleType.ROLE_SUPPLIER]
            return effectRoles(user).any { roles.contains(it.roleType) }
        }
        return false;
    }
