...
import com.cimbidia.wm.core.ApiException
...
import com.cimbidia.wm.product.ProductDetailsCommand;
...
class ProductService {
	...
	def getNdcResults(operation, q) {
		...
		if (!hipaaspaceApiKey) {
			throw new ApiException(message: "Your administrator must specify Hipaaspace.com API key (hipaaspace.api.key) in configuration file (cimbidia-config.properties).  Sign up at <a href='http://www.hipaaspace.com/myaccount/login.aspx?ReturnUrl=%2fmyaccount%2fdefault.aspx' target='_blank'>hipaaspace.com</a>.")
		}
		try {
	...
	def findGoogleProducts(search) {
		...
		if (!googleProductSearchKey) {
			throw new ApiException(message: "Your administrator must specify Google API key (google.api.key) in configuration file (cimbidia-config.properties).  For more information, see Google's <a href='https://developers.google.com/shopping-search/v1/getting_started#getting-started' target='_blank'>Getting Started</a> guide")
		}
    ...
    */
	...
	List<String> findProduct(String productGroup)
	{
		List<ProductGroup> loc = ProductGroup.createCriteria().list {
			and  {
